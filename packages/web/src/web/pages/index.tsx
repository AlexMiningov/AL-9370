...
function About() {
  const items = [
    ["01", "ПОДБОР ОБОРУДОВАНИЯ", "Выберу то, что реально окупается при вашем бюджете и тарифе."],
    ["02", "РАСЧЁТ ДОХОДНОСТИ", "Честный расчёт с учётом сложности, курса и затрат на электричество."],
    ["03", "НАСТРОЙКА И ЗАПУСК", "Конфигурация, разгон, пул — полностью под ключ."],
    ["04", "ОПТИМИЗАЦИЯ ЗАТРАТ", "Снижаю расходы на электроэнергию и обслуживание."],
    ["05", "МАСШТАБИРОВАНИЕ", "Помогаю вырасти от одного ASIC до промышленной фермы."],
    ["06", "УДАЛЁННОЕ УПРАВЛЕНИЕ", "Мониторинг и управление фермой из любой точки мира."],
    ["07", "ПОДБОР ПОМЕЩЕНИЙ", "Оценю локацию по тарифам, охлаждению и безопасности."],
    ["08", "ИНВЕСТИЦИОННАЯ СТРАТЕГИЯ", "Помогу выбрать точку входа в зависимости от фазы рынка."],
  ];
...
function Prices() {
  const rows = [
    { svc: "КОНСУЛЬТАЦИЯ", price: "ОТ $100", note: "60–90 МИН, РАЗОВАЯ СЕССИЯ" },
    { svc: "ЗАПУСК ПОД КЛЮЧ", price: "ИНДИВИДУАЛЬНО", note: "ПОД МАСШТАБ И БЮДЖЕТ" },
    { svc: "АУДИТ ФЕРМЫ", price: "ОТ $300", note: "ПОЛНЫЙ АНАЛИЗ + РЕКОМЕНДАЦИИ" },
    { svc: "ПОЛНОЕ СОПРОВОЖДЕНИЕ", price: "ПО ДОГОВОРЁННОСТИ", note: "ЕЖЕМЕСЯЧНЫЙ АБОНЕМЕНТ" },
  ];
